...
class image_predictor():
	DETC_TRHESH = 0.98#The threashold for it being a face (higher = more sensitive) This is used when we need to detect more than one face in the image.harry: 1.5e11 1d: 1.5e4
	...
	def detect_faces(self, image, scale):
		...
		init_boxes_inds = np.argwhere(classes.flatten()>self.DETC_TRHESH).flatten()
		print(positions)
		init_boxes_pos = np.array(positions)[init_boxes_inds]
		init_boxes = [[i[0],i[1],self.IM_WIDTH,self.IM_HEIGHT] for i in init_boxes_pos]
		print("predictinos are:", classes.tolist())
		
		top_k_boxes = self.draw_top_k_rectangles(image,classes,positions,self.NUM_FACES)
		top_k_boxes_coords = [[i[0],i[1],i[0]+self.IM_WIDTH,i[1]+self.IM_HEIGHT] for i in top_k_boxes]
		boxes_nms = self.non_max_suppression(np.asarray(top_k_boxes_coords),0.28)
		return top_k_boxes, boxes_nms
...
if __name__ == "__main__":
	...
	image = cv2.imread("women_2.jpeg")
	norm_image = cv2.normalize(image, None, alpha=0, beta=1, norm_type=cv2.NORM_MINMAX, dtype=cv2.CV_32F)

	old_height, old_width, _ = image.shape

	scale_factor, n_width, n_height = x.scale_image_to_face_size(image, 50, 800)
